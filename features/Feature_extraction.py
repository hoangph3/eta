...
class Feature_extraction():
    columns = ["ts","flow_duration","Header_Length",
              ...
              "Duration","src_ip_bytes","dst_ip_bytes","src_pkts","dst_pkts", "Rate", "Srate", "Drate"
               ...
               "ack_count", "syn_count", "fin_count", "urg_count", "rst_count", "Spkts", "Dpkts", "Sbytes", "Dbytes", "TnP_per_dport",
               "N_IN_Conn_P_Src_IP", "N_IN_Conn_P_Dst_IP", "TnP_per_Proto", "AR_P_Proto_P_SrcIP", "AR_P_Proto_P_Dst_IP", "AR_P_Proto_P_sport", "AR_P_Proto_P_dport",
    ...
    def pcap_evaluation(self,pcap_file,csv_file_name):
        ...
        protcols_count = {}  # saving the number of packets per protocol
        protcols_count["UDP"], protcols_count["TCP"] = 0, 0
        ...
        incoming_packets_src = {}  # saving the number of incoming packets per src ip
        incoming_packets_dst = {}  # saving the number of incoming packets per dst ip
        ...
        ar_p_proto_p_src_sport = {}
        ar_p_proto_p_dst_dport = {}
        ...
        for ts, buf in pcap:
            ...
            if eth.type == dpkt.ethernet.ETH_TYPE_IP or eth.type == dpkt.ethernet.ETH_TYPE_ARP:
                ...
                new_row = {"ts": ts, 'Source IP': src_ip, 'Destination IP': dst_ip, 'Source Port': src_port, "TnP_per_Proto": json.dumps(packets_per_protocol),
                           'Destination Port': dst_port, "Protocol_name": protocol_name, "Duration": duration
                       , 'Protocol Type': proto_type, "flow_duration": flow_duration, "Sbytes": src_to_dst_byte, "Dbytes": dst_to_src_byte,
                          "Header_Length": flow_byte, "src_ip_bytes": src_byte_count, "fin_flag_number": flag_valus[0],"syn_flag_number":flag_valus[1],"rst_flag_number":flag_valus[2]
               ,"psh_flag_number": flag_valus[3],"ack_flag_number": flag_valus[4],"urg_flag_number": flag_valus[5],"ece_flag_number":flag_valus[6],"cwr_flag_number":flag_valus[7],
                           "dst_ip_bytes": dst_byte_count, "src_pkts": src_pkts, "dst_pkts": dst_pkts, "Rate": rate, "Srate": srate, "Drate": drate
                           , "TNP_per_proto_tcp": protcols_count["TCP"], "TNP_per_proto_udp": protcols_count["UDP"], "Spkts":src_to_dst_pkt, "Dpkts":dst_to_src_pkt
                           ,"ack_count":ack_count, "syn_count":syn_count, "fin_count": fin_count, "urg_count": urg_count, "rst_count": rst_count,
                           "max_flow_duration": max_duration,"min_flow_duration": min_duration,"sum_flow_duration": sum_duration,"average_flow_duration": average_duration,"std_flow_duration": std_duration
                           , "AR_P_Proto_P_SrcIP": json.dumps(AR_P_Proto_P_SrcIP), "AR_P_Proto_P_Dst_IP": json.dumps(AR_P_Proto_P_Dst_IP), "AR_P_Proto_P_sport":json.dumps(ar_p_proto_p_src_sport), "AR_P_Proto_P_dport":json.dumps(ar_p_proto_p_dst_dport)
                           ,"TnP_per_dport":json.dumps(dst_port_packet_count), "N_IN_Conn_P_Src_IP":json.dumps(incoming_packets_src), "N_IN_Conn_P_Dst_IP":json.dumps(incoming_packets_dst),
                           "MQTT": mqtt, "CoAP": coap, "HTTP": http, "HTTPS": https, "DNS": dns, "Telnet":telnet,
                           "SMTP": smtp, "SSH": ssh, "IRC": irc, "TCP": tcp, "UDP": udp, "DHCP": dhcp,
                           "ARP": arp, "ICMP": icmp, "IGMP": igmp, "IPv": ipv, "LLC": llc,
                           "Tot sum":sum_packets, "Min": min_packets, "Max": max_packets, "AVG": mean_packets, "Std": std_packets,
                           "Tot size": ethernet_frame_size, "IAT": IAT, "Number": len(ethsize), "MAC": mac,
                           "Magnitue": magnite, "Radius":radius, "Covariance":covaraince, "Variance":var_ratio, "Weight": weight,
                           "Correlation": correlation, "RARP": rarp, "Wifi_Type":type_info,"Wifi_Subtype":sub_type_info,"DS status":ds_status,
                           "Fragments":fragments,"wifi_src":src_mac,"wifi_dst":dst_mac,"Sequence number":sequence,"Protocol Version": pack_id,
                           "flow_idle_time":idle_time,"flow_active_time":active_time}

                self.GH = self.GH.append(new_row, ignore_index=True)
                self.GH.to_csv(csv_file_name+".csv")
                print("added")
